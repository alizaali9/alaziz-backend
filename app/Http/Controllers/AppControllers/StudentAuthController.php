...
class StudentAuthController extends Controller
{
    public function register(Request $request)
    {
        ...
        $currentYear = date('Y');
        $studentCount = Student::whereYear('created_at', $currentYear)->max('roll_no');

        if ($studentCount) {
            $studentCount = intval(substr($studentCount, -2)) + 1;
        } else {
            $studentCount = 1;
        }
    ...
    public function getStudentByRollNumber(Request $request)
    {
        ...
        $student->picture = $student->picture ? Storage::disk('google')->url($student->picture) : null;
    ...
    public function manage(Request $request)
    {
        ...
        foreach ($students as $student) {
            $student->picture = Storage::disk('google')->url($student->picture);
    ...
    public function uploadPicture(Request $request)
    {
        ...
        if ($request->hasFile('picture')) {
            ...
            $profilePath = env('GOOGLE_DERIVE_FOLDER_NAME') . '/students/student_' . uniqid() . '.' . $extension;
            Storage::disk('google')->put($profilePath, $fileData);
            Storage::disk('google')->setVisibility($profilePath, 'public');

            $picturePath = Storage::disk('google')->url($profilePath);

            return response()->json([
                'status' => 200,
                'message' => 'Picture uploaded successfully',
                'picture_url' => $profilePath,
            ], 200);
        }
